...
/**
 * Brand-specific push notification logic for 5Star Ultra.
 */
object PushNotifications {
    ...
    fun init(app: Application) {
        ...
        try {
            ...
            // FirebaseApp is initialized by the google-services plugin's ContentProvider,
            // but we call getInstance() to ensure it's ready.
    ...
    private fun registerForegroundListener() {
        EventBus.subscribe(NotificationCreatedEvent::class.java) { event ->
            ...
            val title = message.header ?: "5Star Ultra"
            ...
            android.os.Handler(android.os.Looper.getMainLooper()).post {
                FivestarBannerState.show(title, body, link)
            }
        }
    }
    ...
    private fun createMigrationChannel(app: Application) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            if (notificationManager != null) {
                // fivestar_notifications_v2 is defined in FivestarNotificationFactory
                val channel = NotificationChannel(
                    FivestarNotificationFactory.CHANNEL_ID,
                    FivestarNotificationFactory.CHANNEL_NAME,
                    NotificationManager.IMPORTANCE_HIGH
                )
                notificationManager.createNotificationChannel(channel)
                Log.d(TAG, "PushNotifications: Migration channel created/verified: ${FivestarNotificationFactory.CHANNEL_ID}")
            }
        }
    }
